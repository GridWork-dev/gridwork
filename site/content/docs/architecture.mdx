---
title: Architecture
description: Truth ownership, append serialization, storage ports, projections, blobs, crash recovery, and contract surfaces.
---

{/* curated-from: docs/architecture.md sha256=1dde11a890f74d14a15c037a86f6931a68fb223ef301f8c32aec27955c795f23 */}
...
> carrying an arbitrary command line. Template environment entries are durable
> `env:NAME` references resolved only inside the resident host. The host clears
> inherited environment before applying the template map, bounds every resident
> grid before allocation, replaces stale start-manager routes, and periodically
> reaps ended local sessions.
>
...
> **Not built:** the workspace — the real multiplexer (stage 5 of the roadmap);
> anything this file says about that surface is specification.
>
> [Threat model](/docs/security) labels each security stance **in force** /
