...
> exist in the tree, are measured against it, and have the certified green run
> recorded below.
...
> the versions pinned at capture time and re-verified for Claude Code 2.1.223 on
> 2026-08-06, because an acceptance test against an unstated capability is
> unfalsifiable.
...
| --- | --- | --- |
| Claude Code | `2.1.223 (Claude Code)` | bidirectional stream-json (`--input-format stream-json --output-format stream-json`) for the transcript; lifecycle/permission hooks for control |
...
From a clean target directory, build both harness binaries before running the
matrix so the Claude `PreToolUse` helper is present:

```text
cargo build -p gwk-parity --bins --locked
cargo run -p gwk-parity --bin parity --locked
```

## Certified matrix (2026-08-06)

Run locally against the live kernel with Claude Code 2.1.223, Codex 0.146.0,
and opencode 1.18.3. The runner reported `all_green: true`:

| engine | lifecycle | status_truth | transcript_ingestion | approval_relay |
| --- | --- | --- | --- | --- |
...
## Inventory notes (captured 2026-08-01; Claude re-verified 2026-08-06)
...
- **Claude Code** 2.1.223 still exposes no machine-readable per-child cost;
  `--forward-subagent-text` still attributes forwarded child messages through
  `parent_tool_use_id`, while `result.usage`, `modelUsage`, and
  `total_cost_usd` remain whole-invocation or per-model aggregates. Hooks still
  run in every mode, including modes that bypass interactive prompting, and the
  CLI exposes their stream projection through `--include-hook-events`.
  `total_cost_usd` remains a client-side list-rate estimate with no stated error
  bound — the ledger stores it as reported and treats token counts as the
  accounting fact. The 2.1.223 changelog contains no stream-json, hook,
  attribution, or cost-reporting change from the previous pin.
