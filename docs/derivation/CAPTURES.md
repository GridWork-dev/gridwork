...
mistake it corrects is worth writing down. An earlier version of this branch cited
`TERM-UNICODE-CORE` for the RESET-mode behavior. That document defines no reset-mode
segmentation rule; it says three separate times that the unset state is undefined, and its
...
citation dressed as a real one `CAP-002`'s note names — an escalation from "the spec
declines to say" to "the spec says". The behavior is real and this row records it as
measured, through this crate's own public API against the engine the build pins, so the
claim is falsifiable by re-running the probe rather than by reading a rule into a document
that refuses to state one. `TERM-UNICODE-CORE` keeps the citations it actually earns: what
the mode requires when set, and that the reset state is undefined — which is the stronger
argument for pinning both sides anyway.
